...
# ----------- Google Gemini client (Google AI Studio)
try:
...
def _normalize_messages_for_gemini(messages):
    """Convert OpenAI-style messages into Gemini contents format."""
    norm = []
    for m in messages:
        role = m.get("role", "user")
        # oh, gemini only supports 'user' and 'model'
        if role == "system":
            role = "user"
        elif role == "assistant":
            role = "model"
        ...
        norm.append({"role": role, "parts": parts})
    return norm
...
def call_gemini(model: str, messages: List[Dict[str, str]]) -> str:
    ...
    contents = _normalize_messages_for_gemini(messages)
    model_obj = genai.GenerativeModel(model_name=model, system_instruction=system_instruction)
    resp = model_obj.generate_content(contents)
    # Prefer resp.text; fallback best-effort
...
def main():
    ...
    ap.add_argument("--model", required=True, help="LLM model name, e.g., gpt-4o, gpt-4.1-mini, gemini-1.5-pro, gemini-1.5-flash.")
    ...
    load_dotenv()
    # Initialize provider client only if using OpenAI; Gemini config happens in call_gemini
    client = None
    ...
    for idx, item in enumerate(flat_items, 1): # DEBUG: limit to first 3 items [:3] rm
        tag = item["tag"]
        ...
        for r in range(max(1, args.num_runs)):
            try:
                raw = call_openai(client, args.model, messages) if args.model.startswith("gpt") else call_gemini(args.model, messages)
            except Exception as e:
